...
export const registerUser = createAsyncThunk(
    ...
    async (data: RegisterFormData, { rejectWithValue }) => {
        try {
            ...
            await axios.post('https://cooking-app-backend.onrender.com/api/auth/register', responseData);
...
export const login = createAsyncThunk(
    ...
    async (body: LoginFormData, { rejectWithValue }) => {
        try {
            const { data } = await axios.post('https://cooking-app-backend.onrender.com/api/auth/login',
                body,
            )
            localStorage.setItem('userToken', data.jwt)
            localStorage.setItem('userEmail', body.email);
            const email = localStorage.getItem('userEmail')
            axios.get("https://cooking-app-backend.onrender.com/api/users/byemail", { params: {email} }).then(res1 => {
